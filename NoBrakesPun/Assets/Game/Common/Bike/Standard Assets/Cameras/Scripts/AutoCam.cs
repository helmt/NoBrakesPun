...
{
    [ExecuteInEditMode]
    public class AutoCam : PivotBasedCameraRig, IPunObservable
    {
        [SerializeField] private float m_MoveSpeed = 5; // How fast the rig will move to keep up with target's position
        [SerializeField] private float m_TurnSpeed = 15; // How fast the rig will turn to keep up with target's rotation
        [SerializeField] private float m_RollSpeed = 0.5f;// How fast the rig will roll (around Z axis) to match target's roll.
        ...
        [SerializeField] private float m_SmoothTurnTime = 0.8f; // the smoothing for the camera's rotation
        ...
        protected override void FollowTarget(float deltaTime)
        {
            ...
            transform.position.Set(transform.position.x, Target.position.y + 5, transform.position.z);
        }
    }
}
